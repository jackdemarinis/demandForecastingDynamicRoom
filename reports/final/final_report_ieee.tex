...
\section{Data and Business Setting}
The data come from D.\ Hotel Suites \& Spa, an independent 60-room, six-floor hotel located at 1 Country Club Road in Holyoke, Massachusetts. The property serves regional travel demand in the Pioneer Valley, sitting near Route 5 and Interstate 91 between Northampton, Amherst, and Springfield, and roughly forty minutes from Bradley International Airport. Inventory is consistently 60 rooms across all reporting periods, which is the property capacity used throughout the modeling pipeline. The hotel currently sets nightly rates manually, using spreadsheets, historical reference rates, and operator judgment, with no automated demand forecast and no automated pricing recommendation. This is a common situation for independent hotels of this size, because commercial revenue management systems are designed for larger chains and tend to be expensive to integrate for a single small property. The MVP described in this paper is being developed for D.\ Hotel Suites \& Spa as an internal, lower-cost alternative to a commercial system. The author has access to the property's operational reports through a family connection (a partner at the hotel), which is the basis for using a real single-property dataset rather than a public benchmark; further detail on the property and the business motivation is given in Appendix~\ref{app:property}.

The raw data consist of operational workbooks exported from the property's reservations and reporting system, including reservation-style guest reports, Block RoomSales reports, and ReservationForecast reports.
...
\section{Property and Business Context}
\label{app:property}
The MVP target property is D.\ Hotel Suites \& Spa, an independent 60-room hotel in Holyoke, Massachusetts. The building has six floors and is located at 1 Country Club Road, near Route 5 and Interstate 91, on the edge of Holyoke Country Club. The property serves Pioneer Valley travel demand, with the most relevant nearby drivers being Northampton (immediately adjacent), Amherst (about fifteen minutes north), Springfield (about fifteen minutes south), and Bradley International Airport (about forty minutes south). The hotel was previously operated under the Country Inn \& Suites Holyoke flag and has since been rebranded as an independent property. Inventory consists of a regular room class and a suite class, totaling 60 rooms; on-property amenities include a spa, indoor pool, fitness center, breakfast service, and a bar and restaurant. The 60-room inventory figure is consistent across every operational reporting period in the dataset and is used directly as the property capacity throughout the modeling and pricing pipeline. The two over-capacity rows flagged in the data-quality audit correspond to dates where the source report shows rooms sold above 60, which is mechanically impossible given the inventory and is therefore treated as an operational reporting anomaly rather than real demand.

The business motivation for this project is direct and specific. The property currently sets nightly room rates entirely by hand, using spreadsheets, historical patterns, and operator judgment. There is no automated demand forecast and no automated pricing recommendation; every adjustment is a manual decision made by the team that runs the property. This is a common situation for independent hotels of this size, because commercial revenue management systems are generally designed for chains or larger inventories and tend to carry setup, integration, and licensing costs that are difficult to justify for a single 60-room property. The MVP described in this paper is intended to fill that gap: it produces internally usable demand forecasts and bounded ADR recommendations from the same operational reports the team already runs, and it deliberately keeps the recommendation step bounded and human-approved rather than fully automated. The author's family connection to a partner at the property is what made it possible to use this particular dataset for an academic project; the data are not publicly available, but the modeling code, evaluation harness, and report-generation pipeline are open at the repository linked in Appendix~\ref{app:repo}, with personally identifiable guest information excluded from the modeling artifacts. The research goal of the proposal --- comparing classical machine learning and deep learning architectures on hotel demand forecasting --- and the practical goal of the property --- replacing fully manual pricing with a model-supported, manager-approved recommendation flow --- are aligned, because both require an accurate forecast that an operator can audit and trust before applying it to live rates.

\section{Repository, Reproduction, and Artifacts}
\label{app:repo}
The full source code, cleaning scripts, modeling pipeline, pricing layer, and report artifacts are publicly available at \url{https://github.com/jackdemarinis/CSC_561_Final_Project}. Raw workbooks live in \texttt{Data/}; cleaned outputs in \texttt{cleaned\_data/}; derived modeling features in \texttt{processed\_data/}; metrics, figures, and the IEEE report under \texttt{reports/}. Source code is split under \texttt{src/} into feature construction, model training, evaluation, and pricing. Tables and figures in the paper are generated from the same CSV and JSON artifacts produced by the scripts.
